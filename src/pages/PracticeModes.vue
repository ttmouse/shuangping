  ...
  <div v-else class="app">
    ...
    <div v-if="wordCard" class="pm-wordcard" :style="{ left: wordCard.x + 'px', top: wordCard.y + 'px' }" @click.stop>
      ...
      <div v-if="youdaoDetail?.explains?.length" class="pm-wc-sents">
        <div class="pm-wc-sents-title">完整释义</div>
...
function openWordCard(e, wd) {
  ...
  fetchYoudaoWordDetail(wd.word).then((detail) => {
    if (seq === youdaoSeq) {
      ...
      if (wordCard.value) youdaoDetail.value = detail
    }
  }).catch(() => {
...
/* 课程卡片网格（设计重审：流式序号徽章 + 标题/描述/状态分层；无阅读按钮；hover 仅上浮不叠边框+阴影） */
.courseGrid {
  display: grid;
...
/* 卡片主体：纵向内容，序号徽章流式在顶部，标题/描述/状态分层 */
